...
import { OverlayFeedback } from '../../components/OverplayFeedback';
...
export function Quiz() {
  ...
  const cardPostion = useSharedValue(0)
  ...
  const fixedProgressStyle = useAnimatedStyle(() => {
    return {
      position: 'absolute',
      zIndex: 1,
      paddingTop: 50,
      ...
      transform: [
        { translateY: interpolate(scrollY.value, [50, 100], [-40, 1]) }
      ]
    }
  })
  ...
  const dragStyles = useAnimatedStyle(() => {
    const rotateZ = cardPostion.value / CARD_INCLINATION;
    return {
      transform : [
        { translateY : cardPostion.value },
        { rotateZ : `${rotateZ}deg`}
      ]
    }
  })
  ...
  const onPan = Gesture.Pan().onUpdate((event)=>{
    ...
    cardPostion.value = moveToLeft ? event.translationX : 0
  }).onEnd((event)=>{
    if(event.translationX < CARD_SKIP_AREA ) {
      runOnJS(handleSkipConfirm)()
    }
    cardPostion.value = withTiming(0)
  ...
  const shakeStyleAnimated = useAnimatedStyle(() => {
    return {
      transform: [{ translateX: interpolate(shake.value, [0, 0.5, 1, 1.15, 2, 2.5, 3], [0, -15, 0, 15, 0, -15, 0]) }]
    }
  })
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View style={fixedProgressStyle}>
